...
`במידה והינך מעוניין בטיפול רפואי עלייך לפנות למפקד. המפקד ייפנה אותך למסדר חופ"ל. מסדר חופ"ל מתקיים אחת ליום כל יום, בהתאם לפוקדות הצבא. \n במסדר חופ"ל תקבל את הטיפול . הייעוץ הרפואי אשר בסמכות החובש, במידה ויש צורך החופ""ל יפנה אותך לרופא היחידה. אצל רופא היחידה תקבל את המשך הטיפול הרפואי - כולל יפול תרופתי, הפניות לבירור נוסף, פטורים וימי מנוחה.`];
let titleFile = [`"זימון תורים"`, `MEDICLICK`, `MEDITICK`];
let textFile = [`באתר זה תוכלו להזמין תור לרופאים מומחים ובדיקות. חשוב להדגיש במידה וקיבלתם הפנייה לרופא מומחה או בדיקה המצריכה הזמנת תור תוכלו להזמין תור במועד ובמקום הנוח לכם והמאושר ע"י המפקדים.`,
 `אתר הפניות למרפאה - תוכלו לשלוח בקשות לחידוש מרשם קבוע, החזרים כספיים על ביקור במיון, ניפוק משקפיים או מדרסים, הפנייה לאחר המלצה. \n הבקשות יגיעו ישירות אל תא הדואר של הצוות הרפואי ותקבלו התייחסות ללא תור רופא. \n על כל פנייה יש וחשוב לעדכן את החופ"ל (חובש פלוגתי) בכדי שיוכל להיות במעקב על בריאותכם`,
 `אתר הרפואה - התיק הרפואי שלך \n באתר זה תוכל לצפות בתוצאות המעבדה שלך, הפניות, תורים שנקבעו, פטורים, חיסונים, ימי מחלה, סיכומי ביקור, מרשמים ותרופות \n בנוסף קישורים לזימון תורים ושליחת בקשות למרפאה`];
...
const rightLessons = (rightNum)=>{
    ...
    if(rightNum == 3 || rightNum == 4 || rightNum == 5) {
        document.getElementById("band-aid").style.top = "45vh";
...
const signin = (event) => {
    ...
    document.getElementById("title-medicalfile").innerText = `שאלת איך אני מממש את אותן זכויות ? \n מעולה ! תכף נגיע לזה אבל קודם כל, תרשם לאתר הרפואה - פורטל מקול הלב`;
    document.getElementById("text-signin").innerHTML = `לחצו על הקישור הבא : <br> <a href="https://meditik.medical.idf.il/home" class="link">הקישור הבא</a> <br> או <br> כנסו לאיזור האישי באתר צה"ל -> רפואה -> MEDITIK אתר רפואה`;
...
const folderFile =(event)=> {
    // NO SHOW
    document.getElementById(`heart-gif`).style.display = "none";
...
const files = (numFile) => {
    ...
    document.getElementById(`text-file`).innerText = textFile[numFile-1];
